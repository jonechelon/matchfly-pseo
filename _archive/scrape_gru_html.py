...
# Caminho do CSV antigo (na raiz) para migração
OLD_CSV_PATH = os.path.join(os.path.dirname(__file__), "voos_atrasados_gru.csv")
...
def main():
    """
    MODIFICAÇÃO 3: Função principal com criação do CSV ANTES de abrir Playwright.
    """
    # MODIFICAÇÃO 3: PRIMEIRA coisa a fazer - criar CSV com timestamp ANTES do Playwright
    print("=" * 70)
    print("🚀 SCRAPER GRU - VERSÃO INTELIGENTE pSEO")
    print("=" * 70)
    print()
    
    print("📂 Criando CSV com timestamp (merge completo de dados históricos)...")
    print(f"   Diretório de trabalho atual: {os.getcwd()}")
    
    try:
        # Cria novo CSV com timestamp
        csv_file_path = create_csv_with_timestamp()
        print()
    except Exception as e:
        print(f"❌ ERRO FATAL: Não foi possível criar o CSV: {e}")
        import traceback
        traceback.print_exc()
        return
    
    # Executa scraping (DEPOIS de criar/carregar CSV)
    ...
    if not scraped_flights:
        ...
        print(f"📄 Arquivo CSV: {os.path.abspath(csv_file_path)}")
        return
    
    print(f"\n" + "=" * 70)
    print("💾 MERGE/APPEND: Processando CSV com dados históricos")
    print("=" * 70)
    
    # Faz merge/append dos novos voos (preserva dados históricos)
    new_flights_count = merge_flights_to_csv(scraped_flights, csv_file_path)
    ...
    print(f"   • Novos voos adicionados (merge): {new_flights_count}")
    print(f"📄 Arquivo CSV: {os.path.abspath(csv_file_path)}")
    print("=" * 70)
    print("✅ Scraping concluído com merge/append!")
    print("=" * 70)
